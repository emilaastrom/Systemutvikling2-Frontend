...
const ChallengecardAddModal: React.FC<ChallengecardModalProps> = ({
  onClose,
}) => {
  ...
  const fetchSuggestions = async () => {
    try {
      const suggestionsData = await apiHandler(
        "challenge",
        "get",
        "/getNewChallenges?number=5"
      );
      const user = await apiHandler("user", "get", "/getUser");
      setSuggestions(suggestionsData.data);
      setIsLoading(false);
      setSelectedDifficulty(customDiffMapping[user.data.defaultDifficulty]);
      calculateDifficultyDays(
        customDiffMapping[user.data.defaultDifficulty],
        0,
    ...
    return (
      ...
      >
        <span className="text-gray-500 text-sm text-start">
          ...
          {customNameMapping[suggestion.type]}
        </span>
  ...
  const calculateDifficultyDays = (
    ...
    if (difficulty === "Vanskelig") {
      ...
      setDifficultyDays(
        Math.floor((suggestions[page].durationDays / 100) * 80)
      );
    } else if (difficulty === "Enkel") {
      setDifficultyDays(
        Math.floor((suggestions[page].durationDays / 100) * 50)
      );
    }
  };
      ...
      >
        ...
        <div className="flex justify-between h-1/2 mb-4 mt-8">
          <button
            ...
            className={`mx-1 md:px-4 py-2 font-bold ${
        ...
        <div className="flex justify-center items-center">
          <button
            className="border-2 border-primary-light p-5"
            onClick={submit}
          >
            Legg til utfordring
          </button>
