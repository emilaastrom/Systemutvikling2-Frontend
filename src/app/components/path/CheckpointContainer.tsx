...
import CheckpointGoalDivider from "./CheckpointGoalDivider";
...
export default function CheckpointContainer() {
  ...
  const worldPositions = useMemo(
    () => calculateWorldPositions(completedChallenges),
    [completedChallenges, calculateWorldPositions]
  );
    ...
    <div>
      {completedChallenges.map((challenge, index) => {
        ...
        if (canRender(worldPos)) {
          //TODO less hardcoded version of goal division marker 
          if (challenge.id === 0) {
            return (
              <CheckpointGoalDivider
                key={challenge.id}
