...
const SidebarItem: React.FC<SidebarItemProps & { isActive: boolean }> = ({ title, onClick, isActive }) => {
  return (
    <div 
      className={`h-10 flex w-full justify-center rounded-lg items-center hover:bg-green-100 hover:cursor-pointer ${isActive ? 'bg-green-200 hover:bg-green-200' : ''}`}
      onClick={onClick}
    >
      {title}
    </div>
...
const Home: React.FC = () => {
    ...
    switch (content) {
      case "Konto":
        return (
          <div>
            <div className="m-10">
            Brukskonto<br />
          <input type="text" placeholder="Kontonr" className="border-2" />
          </div>
      ...
      case "Badges":
        return (
          ...
          <Image src="/logo.png" height={100} width={100}  alt="badge" className="rounded-full border-2"></Image>
          <Image src="/logo.png" height={100} width={100}  alt="badge" className="rounded-full border-2"></Image>
          <Image src="/logo.png" height={100} width={100}  alt="badge" className="rounded-full border-2"></Image>
          </div>
        );
      case "Innstillinger":
        return (
        <div>
          <input type="text" placeholder="instilling" className="m-10 border-2" />
          </div>
      );
      default:
        return (
  ...
  return (
    <main className="bg-green-100 w-screen h-screen">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 h-auto py-20 md:px-40 px-4">
        {/* Profile section */}
  ...
  <Image src="/default.png" width={100} height={100} alt="Picture" className="md:ml-10 ml-5 rounded-full border-2 border-green-200"></Image>
  <div id="userInfo" className="flex-grow md:ml-10 ml-5">
    <h2 className="text-xl font-bold whitespace-nowrap">{realName}</h2>
    {username}
  </div>
  <Image src="/logo.png" width={200} height={200} alt="Picture" className="hidden md:inline"></Image>
</div>
